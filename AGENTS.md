...
`2.4.3` — `package.json`, `src-tauri/tauri.conf.json`, `src-tauri/Cargo.toml`, `index.html`
What's New dialógus, `README.md`, `Visual Assembler Manual.md`, `Visual Assembler Manual.hu.md`, `INSTALL-MAC.md`,
`INSTALL-LINUX.md`, `README.txt`, `CLAUDE.md`, ez a fájl + copilot-instructions.md.

Fő 2.4.3 változások: **SID Editor effekt-oszlop** — minden tracker cella hordozhat egy effektet a note mellett: vibrato (**V**), slide fel/le (**U**/**D**), note-cut (**C**), sebesség-váltás (**F**) (`_SID_FX_CODES`/`_SID_FX_NAMES`, `_sidApplyRowEffect`, `_sidVoiceState`), amik helyesen játszódnak vissza mind a Web Audio előnézetben, mind a fordított 6502 exportban (`_sidExportAsmPlayable` bővített zero-page táblával, `$02`–`$2F` + `$FB`–`$FE`). **SID Editor song/order list** — minták egy song/order list-en keresztül vannak elrendezve a nyers pattern bank felett, ismételhetők, a dal hosszabb lehet a tárolt minták számánál; a mentett formátum verziójelzőt hordoz, régi (effekt nélküli) `.bin` mentések továbbra is betölthetők. Új `tests/sid-effects.test.js` és `tests/sid-song.test.js`. **D64 Editor explicit Save workflow** — a szerkesztések (add/extract/rename/delete, Block Editor byte-szerkesztés) mostantól egy munkapéldányon landolnak (`_d64EdState.path` vs. `_d64EdState.origPath`, `_d64EdState.dirty`), és csak expliciten Save-elve (`_d64EdSave`/`_d64EdSaveAs`) íródnak vissza a valódi fájlba, automatikus biztonsági mentéssel (`_d64EdBackupDisk`) és mentetlen változás esetén megerősítési kéréssel (`_showConfirm(t("d64EditorUnsavedConfirm"))`); `_d64EdCleanupTemp` takarítja a munkapéldányt. **Block Editor hex grid** — a Block Editor mostantól új byte-onkénti hex griddel jeleníti meg a kiválasztott 256 byte-os blokkot, és a dialógusa már nem homályosítja el az app többi részét (`.dbe-dialog::backdrop`).

Korábbi 2.4.2 változások: **D64 Editor drag & drop** — bináris (`.prg`/`.bin`, pl. a SID/sprite/char szerkesztők exportja) közvetlenül a lemezképre húzható, ez megnyitja a Hozzáadás panelt előre kitöltve; több fájl egyszerre húzva sorban egymás után jön fel. **Include fájl generálása** — új toolbar gomb, ami `.const NÉV = $CÍM` sort ír egy `.inc` fájlba minden lemezen lévő bejegyzéshez, a betöltési címet a session alatt megadott (vagy Exomizer előtt rögzített) értékből, PRG-knél szükség esetén a fájl header-jéből. **Blokk szerkesztő** — új toolbar-kapcsoló, ami a D64 Editort nyers track/sector szintre kapcsolja a directory absztrakció alá: kattintható blokktérkép (BAM szabad/foglalt bitmap szerint színezve, a BAM és a directory lánc külön jelölve), hex nézet/szerkesztő a kiválasztott 256 byte-os blokkhoz, előző/következő szektor navigáció. Új Rust `write_bin_file` Tauri command (a meglévő `read_bin_file` párja) — a track/sector cím-aritmetika és a BAM parse teljesen a JS oldalon van, `c1541`-t nem használ.

Korábbi 2.4.1 változások: **Lengyel és olasz UI nyelv** — teljes felület-fordítás (menük, dialógusok, mnemonik leírások, Ultimate Basic parancsreferencia) a meglévő magyar/angol/spanyol/német/holland mellé. **Online Help toolbar gomb** — új ikon-gomb a Debug után, a c64va.tech/docs.html többnyelvű dokumentációját nyitja meg az appból. **Mnemonik leírások kiszervezése** — a `mnemonicDescriptions*` táblák az `app.js`-ből az `i18n.js`-be kerültek a többi UI-szöveg mellé, egyszerűsítve a nyelvválasztó logikát; ezzel együtt javítva egy régóta fennálló hiba, ahol a `PRINT`, `PRINT_HEX`, `CLEAR_SCREEN`, `WAIT_KEY`, `SET_BORDER`, `SET_BG`, `IRQ_SETUP` és `RAND` angol leírásait egy véletlen spanyol duplikáció felülírta.

Korábbi 2.4.0 változások: **D64 Editor** — új toolbar-ikon (a Curve Editor mögött) egy önálló lemezkép-szerkesztő dialógushoz, a Sprite Editor stílusát követve (Files ▾ menü: New/Open/Save As/Run in VICE). Meglévő `.d64` directory listázása, program hozzáadása/kimentése/átnevezése/törlése közvetlenül a lemezképen `c1541`-en keresztül (nincs külön mentés lépés). Hozzáadásnál opcionális betöltési cím, kicsomagolási cím és Exomizer tömörítés (ua. mem/sfx mód mint a Build D64 extra fájloknál), plusz PRG/SEQ/USR/REL típusválasztó — `.prg`-nél ezek automatikusan rejtve maradnak. A directory lista a C64Pro fonttal, nagybetűsen jelenik meg (`LOAD"$",8` hatás). Javítva: átnevezésnél az input mezőbe kattintás nem törli többé azonnal a szerkesztést. Javítva: világos témában a mód-jelző badge (BLOCK MODE stb.) sötétebb/olvashatóbb, a csillanás animáció ismét látszik. Toolbar: szeparátorok a Tutorials gomb körül és az új D64 Editor ikon után.

Korábbi 2.3.9 változások: `*` (program counter) minden operandus-kifejezésben (`BNE *-5`, `JMP *+20`, `LDA #<*`, `LDA #>(*+63)`; érték után álló `*` = szorzás). Lokális (pontos) labelek: `.loop` a legközelebbi fölötte lévő globális label scope-jába tartozik, a `parseAsmText` már nem vágja le a pontot. Long branch pseudo-opok: `LBNE`/`LBEQ`/`LBCC`/`LBCS`/`LBMI`/`LBPL`/`LBVC`/`LBVS` (`HosszuUgrasok` kategória) → invertált `Bxx *+3` / `JMP` = fix 5 byte. `.assert <expr>[, "üzenet"]` — fordítási idejű ellenőrzés, hamis → beszédes hiba az értékkel. SMC operandus-label: `LDA value:#$00` → `value` az operandus byte-jára mutat. Tartományon kívüli branch a pontos túllépést és a megfelelő `LBxx`-et javasolja.

Korábbi 2.3.8 változások: Munkaterület mentés/megnyitás (`.vaws` fájl) — a megnyitott, lemezen lévő fájlokhoz kötött fülek pontos halmazát, az aktív fület és minden fül szerkesztőmódját menti/tölti vissza, auto-save-lel és induláskor auto-restore-ral; globális memória panel toggle; az Ultimate Basic parancsreferencia (autocomplete + Commands panel) mostantól nyelvfüggő (HU/EN/ES/DE/NL, angol fallback); frissített UB grafikus parancs dokumentáció (COLOR PEN, plot/line/rect/circle, multicolor); javított KERNAL referencia (SETLFS/PLOT cím és hívási konvenció); magas memóriahasználat javítva sok nyitott fül esetén — a per-tab undo/redo history mostantól korlátozott (debounce-szal); felesleges Expert/UB breakpoint toolbar gombok eltávolítva (a breakpoint továbbra is a sorszám-gutterből állítható); Expert toolbar magasság igazítva a UB toolbarhoz.
...
Korábbi 2.3.5 változások: CHARDEF és BOX_HIT makrók szintaxiskiemelése az Expert szerkesztőben (eddig színezetlenek voltak); az Ultimate Basic szerkesztő az Expert szerkesztővel egységes viselkedése (Format utáni kurzorpozíció javítás, egységes Tab és kurzorkövetés select/focus eseményekre); UB betűméret (zoom) megőrzése munkamenetek között; a welcome dialogba bekerült egy Ultimate Basic manual gomb, ami ugyanazt a kézikönyvet nyitja, mint a szerkesztő manual gombja.
