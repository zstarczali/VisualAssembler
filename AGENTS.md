...
`2.4.1` — `package.json`, `src-tauri/tauri.conf.json`, `src-tauri/Cargo.toml`, `index.html`
...
Fő 2.4.1 változások: **Lengyel és olasz UI nyelv** — teljes felület-fordítás (menük, dialógusok, mnemonik leírások, Ultimate Basic parancsreferencia) a meglévő magyar/angol/spanyol/német/holland mellé. **Online Help toolbar gomb** — új ikon-gomb a Debug után, a c64va.tech/docs.html többnyelvű dokumentációját nyitja meg az appból. **Mnemonik leírások kiszervezése** — a `mnemonicDescriptions*` táblák az `app.js`-ből az `i18n.js`-be kerültek a többi UI-szöveg mellé, egyszerűsítve a nyelvválasztó logikát; ezzel együtt javítva egy régóta fennálló hiba, ahol a `PRINT`, `PRINT_HEX`, `CLEAR_SCREEN`, `WAIT_KEY`, `SET_BORDER`, `SET_BG`, `IRQ_SETUP` és `RAND` angol leírásait egy véletlen spanyol duplikáció felülírta.
