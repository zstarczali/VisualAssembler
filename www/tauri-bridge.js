(function () {
  ...
  window.electronAPI = {
    ...
    chooseD64Open: () => invoke("choose_d64_open"),
    chooseD64New: () => invoke("choose_d64_new"),
    d64Format: (path, diskName) => invoke("d64_format", { path, diskName }),
    d64List: (path) => invoke("d64_list", { path }),
    d64AddFile: (path, bytes, name, loadAddress, fileType) => invoke("d64_add_file", { path, bytes, name, loadAddress: loadAddress ?? null, fileType: fileType || "prg" }),
    d64DeleteFile: (path, name) => invoke("d64_delete_file", { path, name }),
    d64RenameFile: (path, oldName, newName) => invoke("d64_rename_file", { path, oldName, newName }),
    d64ExtractFile: (path, name) => invoke("d64_extract_file", { path, name }),
    d64CopyAs: (source, dest) => invoke("d64_copy_as", { source, dest }),
    d64Run: (path) => invoke("d64_run", { path }),
