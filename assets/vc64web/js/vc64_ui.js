...
let play_sound = function(audio_buffer, sound_volume=0.6, sound_name=""){
        if(audio_buffer== null)
        {                 
            load_all_sounds()
                .then(() => {
                    if(sound_name && soundBufferResolvers[sound_name])
                    {
                        const loaded_buffer = soundBufferResolvers[sound_name]();
                        if(loaded_buffer != null)
                        {
                            play_sound(loaded_buffer, sound_volume);
                        }
                    }
                })
                .catch(console.error);
...
const soundBufferResolvers = {
    audio_df_insert: () => audio_df_insert,
    audio_df_eject: () => audio_df_eject,
    audio_df_step: () => audio_df_step,
    audio_key_standard: () => audio_key_standard,
    audio_key_backspace: () => audio_key_backspace,
    audio_key_space: () => audio_key_space
};
...
load_all_sounds().catch(console.error);
...
function message_handler_queue_worker(msg, data1, data2)
{
    ...
    {
        play_sound(audio_df_insert, drive_loudness, "audio_df_insert");
        floppy_has_disk=true; 
    }
    else if(msg == "MSG_DISK_EJECT")
    {
        $("#drop_zone").html(`file slot`);
        play_sound(audio_df_eject, 0.6, "audio_df_eject"); 
    }
        ...
        )
    {
        if(msg == "MSG_DRIVE_STEP")
        {
            ...
            { 
                play_sound(audio_df_step, drive_loudness, "audio_df_step");
...
function InitWrappers() {
    ...
    connect_audio_processor = async () => {
        ...
        try {
            ...
            console.warn("AudioWorklet mono failed, switching to main-thread audio.", e);
            current_audio_device = 'main thread (mono)';
            $("#button_audio_device").text(current_audio_device);
            audio_connected = false;
            try {
                Module._wasm_open_main_thread_audio();
            }
            catch(fallback_error) {
                console.error(fallback_error);
            }
            return;
        }
...
//        worklet_node.connect(audioContext.destination);        
    ...
    connect_audio_processor_stereo = async () => {
        ...
        try {
            ...
            console.warn("AudioWorklet stereo failed, switching to main-thread audio.", e);
            current_audio_device = 'main thread (mono)';
            $("#button_audio_device").text(current_audio_device);
            audio_connected = false;
            try {
